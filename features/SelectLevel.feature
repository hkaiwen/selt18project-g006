Feature: Allow user to select the levels when the user is logged in
  ...
  Scenario: User selects Hard level when logged in and get the scores based on level
    ...
    Then I select Hard level
    Then I select the correct answer
    Then I should see Score: 4 based on level

  Scenario: User selects Easy level when logged in and score should not be updated for incorrect answer
    When I log in with valid user credentials
    Then I select Easy level
    Then I select the incorrect answer
    Then I should see Score: 1 based on level

  Scenario: User selects Medium level when logged in and score should not be updated for incorrect answer
    When I log in with valid user credentials
    Then I select Medium level
    Then I select the incorrect answer
    Then I should see Score: 1 based on level

  Scenario: User selects Hard level when logged in and score should not be updated for incorrect answer
    When I log in with valid user credentials
    Then I select Hard level
    Then I select the incorrect answer
    Then I should see Score: 1 based on level

  Scenario: User selects Easy level when logged in and should not see submit button once option is clicked
    When I log in with valid user credentials
    Then I select Easy level
    Then I select the correct answer
    Then I should not see the submit button

  Scenario: User selects Medium level when logged in and should not see submit button once option is clicked
    When I log in with valid user credentials
    Then I select Medium level
    Then I select the incorrect answer
    Then I should not see the submit button
